...
| --- | --- |
| Mã tài liệu | `SDD-MKTPLACE-CORE-v1.0` |
| Phiên bản | `1.0.0` |
| Trạng thái | ☑ Approved |
| Ngày tạo | 06/06/2026 |
...
> **Mô hình 3 tài liệu:** SDD này (SAD — tầng kiến trúc) tham chiếu _lên_ Standard (chuẩn org-wide) và _xuống_ các **Tech Spec** (thiết kế chi tiết tầng 3). Đặc tả API → OpenAPI; schema cột → Tech Spec/migration; giá trị & secret → config/Vault. Bảng tương ứng SAD↔Tech Spec ở Phụ lục A.
...
* **API-First** – contract rõ ràng (REST ngoài, gRPC nội bộ giữa context).
...
## 2.2 Sơ đồ kiến trúc cấp cao (C4 Level 2 – Container Diagram)

> Có công nghệ + external systems ở rìa + ranh giới tin cậy.

```mermaid
graph TD
    subgraph Internet [INTERNET - untrusted]
        Client[Client Apps: Buyer/Merchant/Admin]
    end
    Gateway[API Gateway / BFF - Kong]

    subgraph VPC [VPC noi bo - service-to-service mTLS]
        Identity[Identity Svc - Node.js+Keycloak]
        Catalog[Catalog Svc - Spring Boot + Elasticsearch]
        Inventory[Inventory Svc - Go]
        Checkout[Checkout Svc - Spring Boot - ORCHESTRATOR]
        Order[Order Svc / OMS - Spring Boot]
        Payment[Payment Svc - Node.js - Escrow/Settlement]
        Notif[Notification Svc - Python]
        Bus{Kafka Event Bus}
    end

    Client -->|HTTPS/REST| Gateway
    Gateway -->|auth/route| Identity
    Gateway --> Catalog
    Gateway --> Checkout
    Gateway --> Order
    Gateway --> Payment

    Checkout ==>|gRPC GetPrice| Catalog
    Checkout ==>|gRPC ReserveStock| Inventory
    Checkout ==>|gRPC CreatePendingOrder| Order
    Checkout ==>|gRPC InitEscrow| Payment
    ...
    Payment ==>|HTTPS+HMAC outbound| PG[Cong thanh toan - external]
    PG -.->|webhook inbound - verify chu ky| Payment
    Payment ==>|payout| Bank[Ngan hang - external]
    Notif ==>|HTTPS| Provider[Email/SMS - external]
```

| Container | Công nghệ | Vai trò | Giao tiếp |
| --- | --- | --- | --- |
| API Gateway/BFF | Kong | Routing, authn, rate limit, TLS | HTTPS → services |
| Identity Svc | Node.js + Keycloak | Authn, RBAC 3 vai trò | OIDC/JWT |
| Catalog Svc | Spring Boot + Elasticsearch | Sản phẩm, kiểm duyệt, search | REST + gRPC + Kafka |
| Inventory Svc | Go | Tồn kho, reserve, trừ kho | gRPC + Kafka |
| Checkout Svc | Spring Boot | Orchestrator: tách đơn, điều phối | gRPC → các context |
| Order Svc (OMS) | Spring Boot | State machine đơn | REST + Kafka |
| Payment Svc | Node.js | Escrow, đối soát, payout | gRPC + Kafka + HTTPS→PG/Bank |
| Notification Svc | Python | Email/SMS | Kafka + HTTPS |
| Event Bus | Kafka | Sự kiện bất đồng bộ | Kafka protocol |
...
flowchart TD
    ...
    %% Processes
    P_Catalog(("Catalog Context")):::process
    P_Inventory(("Inventory Context")):::process
    P_Order(("Order Context")):::process
    P_Payment(("Payment Context")):::process
    ...
    P_Inventory -->|"7. Ghi đè biến động tồn kho"| D_Inventory
...
flowchart TD
    ...
    %% Processes
    P_Checkout((Checkout Context)):::process
    P_Catalog((Catalog Context)):::process
    P_Inventory((Inventory Context)):::process
    P_Order((Order Context)):::process
    P_Payment((Payment Context)):::process
...
| Event | Publisher | Subscriber(s) | Schema tóm tắt | Retry/DLQ |
| --- | --- | --- | --- | --- |
| ProductCreated | Catalog | Inventory | {productId, skus[]} | 3 retries, catalog-dlq |
| PaymentReceived | Payment | Order, Notification | {orderId, txnId, amount} | 5 retries, pay-dlq |
| OrderCompleted | Order | Inventory, Payment | {orderId, items[]} | 5 retries, order-dlq |
...
graph TB
    ...
    subgraph B2[Ranh gioi 2 - Inter-context: mTLS + service account]
        G1[Catalog.GetPrice]:::vpc
        G2[Inventory.ReserveStock]:::vpc
        G3[Order.CreatePendingOrder]:::vpc
        G4[Payment.InitEscrow]:::vpc
    end
    subgraph B3[Ranh gioi 3 - Outbound: HMAC + egress han che + secret Vault]
        O1[to Cong thanh toan]:::out
        O2[to Ngan hang - payout]:::out
    end
    subgraph B4[Ranh gioi 4 - Inbound webhook: verify chu ky + allowlist IP + idempotency]
...
| Inter-context (gRPC) | VPC nội bộ | mTLS + service account |
...
OpenTelemetry → Tempo; đặc biệt quan trọng vì checkout đi qua ≥4 context; 100% errors, 5% normal; propagate trace context qua gRPC + Kafka.
...
| OpenAPI / proto | Hợp đồng API đầy đủ | [Link] |
...
Escrow (giữ tiền trung gian) · Settlement (đối soát) · Payout (chi trả Merchant) · WORM (Write Once Read Many) · Saga · Bounded Context · Tenant isolation · ADR · mTLS · PII.
...
| 5 | ADR liên quan tham chiếu (ADR-0012) | ☑ |
| 9 | Không có credential trong tài liệu | ☑ |
| — | Mọi mục Optional không dùng đã ghi "N/A — lý do" (Mục 11 AI) | ☑ |
