package vn.marketplace.checkout.architecture;
...
/**
 * Architectural fitness functions (fitness-funcs.txt) — automated, deployment-gating checks. The
 * event-publish rule is vacuous for Checkout (no msfw Repository writes — stateless saga), but it
 * stays armed so any future persistent use-case is forced onto the outbox path.
 */
class FitnessFunctionsTest {

    private static JavaClasses checkoutClasses;

    @BeforeAll
    static void importClasses() {
        checkoutClasses = new ClassFileImporter()
                .withImportOption(ImportOption.Predefined.DO_NOT_INCLUDE_TESTS)
                .importPackages("vn.marketplace.checkout");
    }

    @Test
    void dependencyInversion_domainDependsOnNothingOutward() {
        noClasses()
                .that().resideInAPackage("vn.marketplace.checkout.domain..")
                .should().dependOnClassesThat().resideInAnyPackage(
                        "vn.marketplace.checkout.application..",
                        "vn.marketplace.checkout.adapter..",
                        "org.springframework..",
                        "jakarta..",
    ...
    void encapsulation_entitiesHaveNoPublicSetters() {
        noMethods()
                ...
                .allowEmptyShould(true) // Checkout models everything as value objects — no msfw Entity yet
                .check(checkoutClasses);
    }
    ...
    void eventPublish_stateWritingUseCasesAreAnnotated() {
        ...
        methods()
                .that().areDeclaredInClassesThat().resideInAPackage("vn.marketplace.checkout.application..")
